并行内核最基本的形式适用于基本的并行操作(例如：可以完全独立地、以任何顺序应用于每一块数据的操作)。通过使用这个形式，我们可以实现对调度的控制。因此，其是一个描述性编程结构的例子——调度决策由实现做出。\par
...
为了保证正确性，必须假定内核可以并行执行。例如，确保对内存的并发访问可被原子内存操作保护(见第19章)，以避免条件竞争。\par
